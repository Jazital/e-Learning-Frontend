...
const Login = () => {
   ...
   const handleOnChange = (event) => {
      ...
      const submitHandler = (e) => {
         ...
         if(
            ((submittedUsername==dbStudentUsername) && (submittedPassword==dbStudentPassword)) // If the login details match student details
            ||
            ((submittedUsername==dbLecturerUsername) && (submittedPassword==dbLecturerPassword)) // Or if the details match lecturer details, signin
            ){
               ...
               <div className="row no-gutters">
                  <div className="col-xl-12">
                     <div className="auth-form">
                        ...
                        {(loginAttempt.loginState == "success") && (<div className="alert alert-success">{loginAttempt.message}</div>)||""}
                        {(loginAttempt.loginState == "failed") && (<div className="alert alert-error">{loginAttempt.message}</div>)||""}
                        <h4 className="text-center mb-4 "> Sign in your account  </h4>
                       
                        <form  action=""  onSubmit={submitHandler}  >
                           <div className="form-group">    <label className="mb-1 ">  <strong>Email</strong> </label>
                           </div>
                              <input type="text" id="username_input" onChange={handleOnChange} className="form-control"    />
                           <div className="form-group">
                              <label className="mb-1 "> <strong>Password</strong>  </label>
                              <input type="password" id="password_input" onChange={handleOnChange} className="form-control"  />
                           </div>
                        {/*
